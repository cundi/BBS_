...
# 板块
class Category(UrlMixin, CreationModificationDateMixin):
    ...
    def get_url_path(self):
        return reverse("bb:category_details", kwargs={"category_id": str(self.pk)})
    ...
    def save(self, *args, **kwargs):
        ...
        CreationModificationDateMixin.save(self, *args, **kwargs)
...
# 2. 话题总数，所有回帖总数
# 3. 最后一个话题，及其作者、时间
class Forum(UrlMixin, CreationModificationDateMixin):
...
# 帖子（话题）
class Topic(UrlMixin, CreationModificationDateMixin):
    # 帖子作者
...
# 对话题（帖子）的回复，即回帖
class Post(UrlMixin, CreationModificationDateMixin):
    # 回帖作者
    author = models.ForeignKey(settings.AUTH_USER_MODEL)
    # 被回复的那个帖子
